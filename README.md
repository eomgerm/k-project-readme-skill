...
<p align="center">
  ...
  기술적 문제 해결은 <strong>기술 서사형</strong>으로, 서비스 경험은 <strong>서비스 소개형</strong>으로 보여주세요.<br/>
  19개 블록 중 저장소 조건이 맞는 것만 <strong>블록처럼 조립</strong>하기 때문에, 없는 내용으로 자리를 채우지 않습니다.
</p>
...
19개 블록은 다섯 단계로 묶여 있고, 문서에는 이 순서대로 들어갑니다.
조건이 안 켜진 블록은 빈 자리를 남기지 않고 그냥 빠집니다.

| 단계 | 블록 | 켜지는 조건 |
| --- | --- | --- |
| **히어로** | 배너 | 항상 |
| | 링크줄 | 항상 (링크가 2개 이하면 소개 문단에 녹입니다) |
| | 스토어 링크 | 앱 저장소 |
| | CI 상태 뱃지 | CI 설정이 있을 때 |
| **요약** | 서비스 소개 + 불릿 | 항상 |
| | 개발 기간 | 항상 |
| | 목차 | 본문이 8KB를 넘을 때 |
| | 기획 의도 | 기획 배경을 답해줬을 때 |
| **주인공** | 기술적 도전 | 기술 서사형 + 전후 수치가 있을 때 |
| | 핵심 기능 | 항상 (기술 서사형에서는 본체 끝으로) |
| **본체** | 기술 스택 | 항상 |
| | 아키텍처 | 항상 (서비스 소개형은 선택) |
| | 폴더 트리 | 기술 서사형 |
| | ERD | 서버 저장소 |
...
| **사람·규칙** | 팀원 | 항상 |
| | 컨벤션 | 실제로 지킨 규칙이 있을 때 |
| | CI/CD 표 | 워크플로가 2개 이상일 때 |

**주인공 단계가 모드 스위치입니다.** 기술 서사형은 기술적 도전이 여기 오고 핵심 기능이 본체 끝으로
내려가며, 서비스 소개형은 핵심 기능이 여기 오고 기술적 도전은 빠집니다.

각 블록의 마크업 골격과 변형은 [블록 카탈로그](skills/k-readme/references/structure.md)에 있습니다.
