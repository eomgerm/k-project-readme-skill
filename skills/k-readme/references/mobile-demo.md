...
- `--time-limit` 기본값 180초. Android 13까지는 이게 상한이고 14부터는 `0`으로 해제된다.
  GIF로 쓸 거면 **10~15초를 넘기지 않는다**
...
| `bayer_scale` | 0~5, 기본 2. 낮을수록 디더링이 강하다. 파일 크기는 단조롭지 않으니(0이 오히려 가장 작을 수 있다) 용량은 아래 "용량 줄이기"로 잡는다. 3이 무난 |
