...
1. 이 서비스를 한 줄로 소개하면?
2. 왜 만들었나? (기획 배경)
3. (부트캠프형) 4번 후보 중 무엇을 쓸까? / (동아리형) 배포 주소와 스토어 링크

### 6. 작성

섹션 순서는 아래 표. 마크업 실물은 `references/structure.md`를 읽어서 쓴다.

| # | 섹션 | 부트캠프형 | 동아리형 |
| --- | --- | --- | --- |
| 1 | 히어로 (배너 + 가운데정렬 링크줄) | 필수 | 필수 |
| 2 | 목차 | 8KB 넘으면 | 8KB 넘으면 |
| 3 | 서비스 소개 | 짧게 | **비중 큼** |
| 4 | 기술적 도전 (문제→시도→결과→링크) | **주인공, 3번 다음** | 생략 가능 |
| 5 | 핵심 기능 (GIF 테이블) | 간략 | **주인공** |
| 6 | 기술 스택 (표) | 필수 | 필수 |
| 7 | 아키텍처 + 폴더 트리 | 필수 | 선택 |
| 8 | 팀원 (아바타 테이블) | 필수 | 필수 |
| 9 | 컨벤션 (브랜치·커밋) | 선택 | 선택 |

부트캠프형은 4번을 5번보다 **앞에** 둔다. 기술 서사가 주인공이면 맨 위에 와야 한다.
...
- 좋음: "200명이 5초간 위치를 갱신할 때 16.7초에서 6.7초로 줄였습니다"
- 좋음: "이거 모잇! 왤케 편해잇!" (팀 개성이 드러나는 표현)
- 나쁨: "혁신적인 사용자 경험을 제공합니다" (중간지대의 홍보체 — 아무도 안 쓴다)

### 이모지

- **헤딩에만 1개.** 문장 속에는 넣지 않는다. 실측이 정확히 이 모양이다(헤딩 120개, 본문 0)
