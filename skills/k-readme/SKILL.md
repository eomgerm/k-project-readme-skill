---
name: k-readme
description: 한국 개발 문화의 "프로젝트 소개서" 형태로 README를 작성한다. 기술 서사형(기술적 도전이 맨 앞)과 서비스 소개형(기능 데모가 맨 앞) 두 모드를 지원한다. "README 작성", "리드미 써줘", "readme 만들어줘", "프로젝트 소개 문서", "깃허브 README", "포트폴리오 README", "리드미 보강", "README 다시 써줘", "한국형 리드미" 요청에 사용. 부트캠프·IT 연합동아리(SOPT·디프만·YAPP·넥스터즈·매쉬업·DDD·코테이토·UMC·부스트캠프·우아한테크코스·카카오테크캠퍼스) 프로젝트 저장소가 주 대상이다.
license: MIT
---
...
**모드는 README 맨 앞에 무엇이 오는지로 갈린다.** 소속이 아니라 그걸로 고른다.

| 모드 | 맨 앞에 오는 것 | 이런 팀이 많이 쓴다 |
| --- | --- | --- |
| **기술 서사형** | 기술적 도전 — 문제·시도·전후 수치 | 부스트캠프, 우아한테크코스, 카카오테크캠퍼스, SSAFY, 소마 |
| **서비스 소개형** | 기능 데모 — 화면 GIF와 사용법 | SOPT, 디프만, YAPP, 넥스터즈, 매쉬업, DDD, 코테이토, UMC |

인자로 받는다: `/k-readme 기술` 또는 `/k-readme 서비스`.
`부트캠프`·`동아리`도 같은 뜻으로 받는다 — 소속으로 부르는 사람이 많다.
인자가 없으면 `AskUserQuestion`으로 한 번 묻되 **소속이 아니라 무엇을 앞에 세울지**를 묻는다.
레포만 보고 추측하지 말 것 — 구분 근거가 없다.
...
### 4. 트러블슈팅 후보 발굴 (기술 서사형에서 필수)

이 섹션이 기술 서사형의 주인공인데 재료는 이미 저장소에 있다.
...
| **L2 주인공** | T1 기술적 도전 | 기술 서사형 + 전후 수치가 있을 때 |
| | F1 핵심 기능 | 항상 (기술 서사형은 L3 끝으로) |
| **L3 본체** | K1 기술 스택 | 항상 |
| | A1 아키텍처 | 항상 (서비스 소개형은 선택) |
| | A3 폴더 트리 | 기술 서사형 (서비스 소개형은 생략) |
| | A4 ERD | 서버 레포 |
| | R1 실행 방법 | 실행할 수 있음 |
| | R2 API 명세 | API 문서 있음 |
| **L4 사람·규칙** | P1 팀원 | 항상 |
...
순서는 레이어 순이다. **L2가 모드 스위치다** — 기술 서사형은 T1이 여기 오고 F1은 L3 끝으로
내려간다. 서비스 소개형은 F1이 여기 오고 T1은 생략한다.

조건이 안 켜진 블록은 자리를 비우지 않고 그냥 빠진다. **기술 서사형인데 전후 수치가 없으면**
...
- **한 파일 안에서 섞어도 된다.** 통일하려 들면 오히려 AI 티가 난다. 실제 README는
  섹션마다 톤이 튄다. 서비스 소개형은 해요체 비중을 높이고, 기술 서사형은 합쇼체로 간다
