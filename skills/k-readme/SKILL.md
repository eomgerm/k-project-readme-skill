...
| API 문서 있음 | **서버 레포일 때만.** `springdoc` `springfox` `swagger-ui-express` `swagger-jsdoc` `@nestjs/swagger` `drf-spectacular` `drf-yasg` `swaggo` `utoipa` 의존성(버전 카탈로그 포함), `docs/**/swagger.json`·`openapi.{yaml,json}`, 또는 `fastapi`(OpenAPI 내장). `swagger-typescript-api` `openapi-typescript` `openapi-fetch` `orval` `openapi3-ts`는 프런트 코드 생성 도구라 근거가 아니다 |
