...
1. 이 서비스를 한 줄로 소개하면?
2. 왜 만들었나? (기획 배경)
3. (부트캠프형) 4번 후보 중 무엇을 쓸까? 위키에 정리한 글이 있으면 주소도 함께 /
   (동아리형) 배포 주소·스토어 링크, 노션·피그마 주소
...
조건이 안 켜진 블록은 자리를 비우지 않고 그냥 빠진다. **L2가 둘 다 안 켜지면**(기여자 1명,
전후 수치 없음) L2를 비우고 L3의 K1·A1을 앞으로 당긴다. 재료 없는 주인공을 억지로 세우면
홍보체가 된다.
